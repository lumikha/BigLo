...
class ChargifyConnector
{
	...
	public function requestCreateMetadata($metadataRequest, $subscription_id, $format = 'XML') {
		$extension = strtoupper($format) == 'XML' ? '.xml' : '.json';
		$base_url = "/subscriptions/" . $subscription_id . "/metadata" . $extension;

		$metadata = $this->sendRequest($base_url, $format, 'POST', $metadataRequest);
		
		if ($metadata->code == 201) { //CREATED			
			return $metadata->response;
		} elseif ($metadata->code == 422) { //UNPROCESSABLE ENTITY
			$errors = new SimpleXMLElement($metadata->response);
			throw new ChargifyValidationException($metadata->code, $errors);  		
		}		
	}
	
	public function createMetadata($chargify_metadata, $subscription_id) {
		$xml = $this->requestCreateMetadata($chargify_metadata->getJSON(), $subscription_id, 'JSON');
		//$metadata = new SimpleXMLElement($xml);
		//return new ChargifyMetadata($metadata);
	}
}
?>